...
import { DR_RESET_TIMER, DR_MULTIPLIERS } from './abilities.js';
import type { BuffDefinition, BuffEffect } from './abilities.js';
...
export class BuffSystem<T extends Positionable> {
  private activeBuffs = new Map<T, ActiveBuff[]>();
  private drState = new Map<T, Map<string, DREntry>>();
  onBuffApplied?: (target: T, definition: BuffDefinition) => void;
  onBuffExpired?: (target: T, definition: BuffDefinition) => void;
  onStacksChanged?: (target: T, buffId: string, oldStacks: number, newStacks: number) => void;

  /** Apply a buff/debuff. Returns false if blocked by DR immunity. */
  apply(target: T, definition: BuffDefinition): boolean {
  ...
  setRemaining(target: T, buffId: string, remaining: number): void {
  ...
  addStacks(target: T, buffId: string, amount: number): void {
    const buffs = this.activeBuffs.get(target);
    if (!buffs) return;
    const buff = buffs.find(b => b.definition.id === buffId);
  ...
  removeStacks(target: T, buffId: string, amount: number): void {
    const buffs = this.activeBuffs.get(target);
    if (!buffs) return;
    const buff = buffs.find(b => b.definition.id === buffId);
  ...
  getStacks(target: T, buffId: string): number {
    const buffs = this.activeBuffs.get(target);
    if (!buffs) return 0;
    const buff = buffs.find(b => b.definition.id === buffId);
  ...
  remove(target: T, buffId: string, silent = false): void {
    const buffs = this.activeBuffs.get(target);
    if (!buffs) return;
  ...
  getAutoAttackSpeedMultiplier(target: T): number {
    const buffs = this.activeBuffs.get(target);
    if (!buffs) return 1;
    let mult = 1;
  ...
  getMovementSpeedMultiplier(target: T): number {
    const buffs = this.activeBuffs.get(target);
    if (!buffs) return 1;
    let mult = 1;
    for (const buff of buffs) {
  ...
  getDamageTakenMultiplier(target: T): number {
    const buffs = this.activeBuffs.get(target);
    if (!buffs) return 1;
    let mult = 1;
    for (const buff of buffs) {
  ...
  getManaCostMultiplier(source: T): number {
  ...
  getDamageDealtMultiplier(source: T): number {
    const buffs = this.activeBuffs.get(source);
    if (!buffs) return 1;
    let mult = 1;
    for (const buff of buffs) {
  ...
  absorbDamage(target: T, incomingDamage: number): { remaining: number; reflectDamage: number } {
    const buffs = this.activeBuffs.get(target);
  ...
  getDRTimers(target: T): { category: string; buffId: string; icon: string; count: number; remaining: number; total: number }[] {
