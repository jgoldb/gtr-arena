import { REGEN_TICK_INTERVAL, MANA_REGEN_DELAY, RESTING_MANA_MULTIPLIER, getCharacterStats } from './characters.js';
import type { Positionable } from './types.js';
import type { BuffSystem } from './BuffSystem.js';

export class RegenSystem<T extends Positionable> {

  private tickAccumulator = 0;
  private manaUsedTimers = new Map<T, number>(); // entity → seconds since last mana use
  private getEntities: () => T[];
  private buffSystem: BuffSystem<T> | null = null;

  constructor(getEntities: () => T[]) {
    this.getEntities = getEntities;
  }

  setBuffSystem(buffSystem: BuffSystem<T>): void {
