...
// ── Collision types (mirror of client CollisionSystem, no Three.js) ─────
...
export interface ResolveResult {
  x: number; z: number; groundY: number; inWater: boolean;
}

interface WaterZone {
  cx: number; cz: number;
  halfW: number; halfD: number;
  surfaceY: number; floorY: number;
}
...
export class CollisionSystem {
    ...
    for (const obs of obstacles) {
      if (obs.type === 'water') {
        ...
        this.waterZones.push({
          cx: obs.position.x, cz: obs.position.z,
          halfW: obs.scale.x / 2, halfD: obs.scale.z / 2,
          surfaceY, floorY: surfaceY - WATER_WADE_DEPTH,
        });
        continue;
      }
      ...
      if (obs.type === 'cylinder') {
        ...
        if (hasZRotation) {
          ...
          this.colliders.push({
            type: 'box',
            cx: obs.position.x, cz: obs.position.z,
            halfW: obs.scale.y / 2, halfD: obs.scale.x,
            cosY: Math.cos(yAngle), sinY: Math.sin(yAngle),
            centerY: obs.position.y, halfH: obs.scale.x, rotZ: 0,
          });
        } else {
          this.colliders.push({
            type: 'circle',
            cx: obs.position.x, cz: obs.position.z,
            radius: obs.scale.x, centerY: obs.position.y, halfH: obs.scale.y / 2,
          });
        }
      } else {
        const yAngle = obs.rotation?.y ?? 0;
        this.colliders.push({
          ...
          halfW: obs.scale.x / 2, halfD: obs.scale.z / 2,
          cosY: Math.cos(yAngle), sinY: Math.sin(yAngle),
          centerY: obs.position.y, halfH: obs.scale.y / 2,
  ...
  resolve(px: number, pz: number, py: number, radius: number): ResolveResult {
    for (let iter = 0; iter < 4; iter++) {
      ...
      for (const col of this.colliders) {
        ...
        if (result) { px = result.x; pz = result.z; pushed = true; }
      }
      if (!pushed) break;
    }
    ...
    for (const wz of this.waterZones) {
      if (px >= wz.cx - wz.halfW && px <= wz.cx + wz.halfW &&
    ...
    for (const col of this.colliders) {
      ...
      if (overlaps && topY > groundY) groundY = topY;
    }
  ...
  addCollider(collider: Collider): void { this.colliders.push(collider); }
  removeCollider(collider: Collider): void {
    const idx = this.colliders.indexOf(collider);
    if (idx !== -1) this.colliders.splice(idx, 1);
  }

  // ── Private helpers (identical to client CollisionSystem) ──────────────
  ...
  private pushCircleVsCircle(px: number, pz: number, radius: number, col: CircleCollider): { x: number; z: number } | null {
    const dx = px - col.cx;
    const dz = pz - col.cz;
    ...
    if (dist < 0.0001) return { x: px + minDist, z: pz };
    const overlap = minDist - dist;
    return { x: px + (dx / dist) * overlap, z: pz + (dz / dist) * overlap };
  }

  private pushCircleVsBox(px: number, pz: number, radius: number, col: BoxCollider): { x: number; z: number } | null {
